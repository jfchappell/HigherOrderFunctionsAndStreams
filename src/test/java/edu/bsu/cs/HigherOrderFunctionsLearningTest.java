...
public class HigherOrderFunctionsLearningTest {
    ...
    public void testMap() {
        ...
        List<Integer> actual = input.map(x -> x + 1).collect(Collectors.toList()); // TODO: Replace the `actual` with your solution, here and in all the following.
    ...
    public void testFilter() {
        ...
        List<String> actual = input.filter(x -> x.charAt(0) == 'b').collect(Collectors.toList());
    ...
    public void testConcatEven() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        String actual = input.filter(x -> Integer.parseInt(x.substring(1, 2)) % 2 == 0).collect(Collectors.joining());
    ...
    public void testCountByDate() {
        ...
        long actual = input.filter(x -> instant.isAfter(x.timestamp)).count();
        int expected = 3;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountNonBots() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input.filter(x -> !x.user.toLowerCase(Locale.ROOT).contains("bot")).count();
        int expected = 26;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountChangesInFebruary() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input.filter(x -> LocalDate.ofInstant(x.timestamp, ZoneOffset.UTC).getMonth() == Month.FEBRUARY).count();
        int expected = 9;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountChangesByMonth() {
        Stream<Revision> input = getRevisions("soup04.json");
        Map<Month, Long> actual = input.collect(Collectors.groupingBy(x -> LocalDate.ofInstant(x.timestamp, ZoneOffset.UTC).getMonth(), Collectors.counting()));
